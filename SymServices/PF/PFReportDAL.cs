...
SELECT ve.Code,ve.EmpName,
      ...
      ,[LoanNo]
  FROM " + HRMDB + @".[dbo].[EmployeeLoan] el 
  Left Outer Join " + HRMDB + @".[dbo].[ViewEmployeeInformation] ve on ve.EmployeeId=el.EmployeeId
...
LEFT OUTER JOIN " + HRMDB + @".[dbo].EmployeeLoanDetail ln 
...
ivn.Name as InvestmentName,
 ...
 Left Join  " + HRMDB + @".[dbo].[FiscalYearDetail] fy on fy.Id=ia.FiscalYearDetailId
where InvestmentTypeId=3
 ...
 iv.InvestmentValue,
  ...
  Left Join  " + HRMDB + @".[dbo].[FiscalYearDetail] fy on fy.Id=ivn.FiscalYearDetailId
...
SELECT
    ...
    '' as [T9]
FROM 
   " + _dbsqlConnection.HRMDB + @".[dbo].View_TIBSalary s
LEFT OUTER JOIN 
    " + _dbsqlConnection.HRMDB + @".[dbo].ViewEmployeeInformation e ON s.EmployeeId = e.EmployeeId
LEFT OUTER JOIN 
    " + _dbsqlConnection.HRMDB + @".[dbo].EmployeePersonalDetail p ON s.EmployeeId = p.EmployeeId
CROSS APPLY (
    VALUES (
...
";
        ...
        SELECT 
             'PF' + e.DCode AS [Account code],
                ...
                '' AS [T9]
        FROM 
            " + _dbsqlConnection.PFDB + @".[dbo].EmployeeForfeiture f
			   LEFT OUTER JOIN 
                " + _dbsqlConnection.HRMDB + @".[dbo].ViewEmployeeInformation e ON f.EmployeeId = e.EmployeeId
        ) 
        SELECT 
            *
        FROM 
