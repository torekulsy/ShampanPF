...
namespace SymRepository.PF
{
    public class PFDetailRepo
    {
        ...
        {
            return new PFDetailDAL().ExportExcelFilePF(Filepath, FileName, ProjectId, DepartmentId, SectionId, DesignationId, CodeF, CodeT, fid, Orderby);
        }

        public string[] ImportExcelFile(string fullPath, string fileName, ShampanIdentityVM vm, int branchId, int FYDId = 0, string PId="")
        {
            try
            {
                return new PFDetailDAL().ImportExcelFile(fullPath, fileName, vm, branchId, null, null, FYDId, PId);
            }
            catch (Exception ex)
            {
                throw ex;
